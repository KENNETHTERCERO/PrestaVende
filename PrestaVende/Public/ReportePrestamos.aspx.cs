        ...
        protected void btnGenerarExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (int.Parse(id_sucuarsal) > 0)
                    ...
                    {
                        string scriptEstadoCuenta = "window.open('WebReport.aspx?tipo_reporte=16" + "&id_sucursal=" + id_sucuarsal + "&fecha_inicio=" + this.txtFechaInicial.Text + "&fecha_fin=" + this.txtFechaFin.Text + "&tipo=excel');";
                        ScriptManager.RegisterClientScriptBlock(this, GetType(), "NewWindow", scriptEstadoCuenta, true);
                    }
                else
                    showWarning("Seleccione una sucursal para poder generar el reporte.");
